...
このルールは`textlint --fix`による自動修正に対応しています。
修正は文の境界に改行を挿入します。
リスト項目内では、継続行のインデントを省いてもリスト項目の内容として扱われるCommonMark仕様の規定（[List Items](https://spec.commonmark.org/0.31.2/#list-items)）に頼らず、項目自体のインデントに合わせて継続行にインデントを付けます。
