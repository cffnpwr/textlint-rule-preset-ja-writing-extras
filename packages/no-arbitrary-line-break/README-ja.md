...
本ルールは`textlint --fix`による自動修正に対応しています。
修正では、改行とその前後の空白（ハードブレイクの行末の`\`を含む）を取り除き、2行を1行に連結します。
取り除いた範囲の直前と直後の文字がどちらも和字でない場合（英単語間など）のみ、半角スペース1つを残します。
それ以外の場合は、間に何も挟まず直接連結します。
